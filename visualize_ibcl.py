...
# Plot metrics
def plot_metrics(dict_metric_1, dict_metric_2, task_nums=5, metric_name='Avg per task accuracy', bt=False):
    if bt:
        task_range = list(range(1, task_nums))
    else:
        task_range = list(range(task_nums))
    task_range = [int(j) for j in task_range]
    plot_data_1 = [dict_metric_1[j] for j in task_range]
    plot_data_2 = [dict_metric_2[j] for j in task_range]

    if bt:
        max_data_1 = [np.amax(plot_data_1[j]) for j in range(task_nums-1)]
        mean_data_1 = [np.mean(plot_data_1[j]) for j in range(task_nums-1)]
        min_data_1 = [np.amin(plot_data_1[j]) for j in range(task_nums-1)]
        
        max_data_2 = [np.amax(plot_data_2[j]) for j in range(task_nums-1)]
        mean_data_2 = [np.mean(plot_data_2[j]) for j in range(task_nums-1)]
        min_data_2 = [np.amin(plot_data_2[j]) for j in range(task_nums-1)]
    else:
        max_data_1 = [np.amax(plot_data_1[j]) for j in range(task_nums)]
        mean_data_1 = [np.mean(plot_data_1[j]) for j in range(task_nums)]
        min_data_1 = [np.amin(plot_data_1[j]) for j in range(task_nums)]
        
        max_data_2 = [np.amax(plot_data_2[j]) for j in range(task_nums)]
        mean_data_2 = [np.mean(plot_data_2[j]) for j in range(task_nums)]
        min_data_2 = [np.amin(plot_data_2[j]) for j in range(task_nums)]

    plt.plot(np.array(task_range) + 1, max_data_1, '-*', label='ZS-IBCL', color='red', markersize=12)
    plt.plot(np.array(task_range) + 1, mean_data_1, '--', color='red')
    #plt.fill_between(np.array(task_range) + 1, max_data_1, min_data_1, facecolor='tab:blue', alpha=0.5)

    plt.plot(np.array(task_range) + 1, max_data_2, '-*', label='FS-IBCL', color='green', markersize=12)
    plt.plot(np.array(task_range) + 1, mean_data_2, '--', color='green')
    #plt.fill_between(np.array(task_range) + 1, max_data_2, min_data_2, facecolor='tab:green', alpha=0.5)
...
if __name__ == '__main__':
    ...
    # Load dict of accs
    dict_all_accs_1 = torch.load(os.path.join(args.data_dir, f'dict_all_accs_{args.alpha}_{args.discard_threshold}.pt'))
    dict_all_accs_2 = torch.load(os.path.join(args.data_dir, f'dict_fs_all_few_accs_{args.alpha}_{args.discard_threshold}.pt'))
    ...
    # Plot metrics
    plot_metrics(dict_avg_accs_1, dict_avg_accs_2, task_nums=task_nums, metric_name='Avg per task accuracy', bt=False)
    plot_metrics(dict_peak_accs_1, dict_peak_accs_2, task_nums=task_nums, metric_name='Peak per task accuracy', bt=False)
    plot_metrics(dict_avg_bts_1, dict_avg_bts_2, task_nums=task_nums, metric_name='Avg per task backward transfer', bt=True)
