...
def train_model(model, train_loader, epochs=50):
    ...
    for epoch in range(epochs):
        for x, y in train_loader:
            optimizer.zero_grad()
            outputs = model(x)
            # Ensure labels are class indices, not one-hot; adjust if using one-hot labels
            y = y.float()
...
def sample_hdr(hdr: dist, model_type='small', train_data= None):
    if model_type == 'small':
        net = BayesianClassifierSmall()
    elif model_type == 'normal':
        net = BayesianClassifier()
    ...
    if train_data:
        train_loader = get_dataloader(data=train_data['data'], labels=train_data['labels'], shuffle=True)
    ...
    for i in range(num_tasks):
        ...
        for j in range(len(hdrs)):  # each pref
            ...
            for k in range(num_models_per_pref):  # each model
                train_data = {'data': np.array(all_data_test[i]), 'labels': np.array(all_label_test[i])}
                test_bnn = sample_hdr(hdrs[j], model_type=model_type, train_data=train_data)
        ...
        if discard_threshold == 0.0:
            torch.save(dict_all_accs, os.path.join(data_dir, f'dict_fs_all_few_accs_{alpha}.pt'))
        else:
            torch.save(dict_all_accs, os.path.join(data_dir, f'dict_fs_all_few_accs_{alpha}_{discard_threshold}.pt'))
